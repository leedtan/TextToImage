...
class GAN:
	...
	def __init__(self, options):
		...
		self.d_bn4_smaller = ops.batch_norm(name='d_bn4_smaller')
	...
	def build_model(self):
		...
		fake_image, fake_small_image = self.generator(t_z, t_real_caption)
		...
		with tf.variable_scope('scope_small'):
			disc_small_real_image, disc_small_real_image_logits   = self.discriminator_smaller(real_small_image, t_real_caption)
			disc_small_wrong_image, disc_small_wrong_image_logits   = self.discriminator_smaller(wrong_small_image, t_real_caption, reuse = True)
			disc_small_fake_image, disc_small_fake_image_logits   = self.discriminator_smaller(fake_small_image, t_real_caption, reuse = True)
		...
		d_loss = d_loss1 + d_loss2 + d_loss3 + g_loss_small + d_loss1_small  + d_loss2_small + d_loss3_small
	...
	def generator(self, t_z, t_text_embedding):
		...
		res1_in = tf.concat( 1, [tf.reshape(h1,[self.options['batch_size'],s8* s8* self.options['gf_dim']*4 ]), z_concat], name='h1_res_block')
		res1block = self.g_bnresblock1(ops.linear(res1_in, 1000))
		
		res1block_applied = ops.linear(tf.nn.relu(res1block,  s4* s4* self.options['gf_dim']*2,'g_res1block_applied'))
		res1block_applied = tf.reshape(res1block_applied, [-1, s4, s4, self.options['gf_dim']*2])
		
		h2 = ops.deconv2d(h1, [self.options['batch_size'], s4, s4, self.options['gf_dim']*2], name='g_h2')
		h2 = tf.nn.relu(self.g_bn2(h2)) + res1block_applied
		...
		smallest_generated = ops.deconv2d(h1, [self.options['batch_size'], s4, s4, 3], name='g_smallest_generated')
		
		return (tf.tanh(h4)/2. + 0.5), (tf.tanh(smallest_generated)/2. + 0.5)
	...
	def discriminator_smaller(self, image, t_text_embedding, reuse=False):
		if reuse:
			tf.get_variable_scope().reuse_variables()
		h0 = ops.lrelu(ops.conv2d(image, self.options['df_dim'], name = 'd_h0_conv')) #32
		h1 = ops.lrelu( self.d_bn1(ops.conv2d(h0, self.options['df_dim']*8, name = 'd_h1_conv'))) #16
		last_layer = h1
		if h1.get_shape()[1] > 5:
			h2 = ops.lrelu( self.d_bn2(ops.conv2d(h1, self.options['df_dim']*8, name = 'd_h2_conv'))) #8
			last_layer = h2
			if h2.get_shape()[1] > 5:
				h3 = ops.lrelu( self.d_bn3(ops.conv2d(h2, self.options['df_dim']*8, name = 'd_h3_conv'))) #4
				last_layer = h3
		...
		h3_concat = tf.concat( 3, [last_layer, tiled_embeddings], name='h3_concat')
		h3_new = ops.lrelu( self.d_bn4_smaller(ops.conv2d(h3_concat, self.options['df_dim']*8, 1,1,1,1, name = 'd_h3_conv_new'))) #4
