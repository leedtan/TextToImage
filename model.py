...
class GAN:
	...
	def discriminator(self, image, t_text_embedding, reuse=False):
		...
		tiled_embeddings = tf.tile(reduced_text_embeddings, [1,32,32,1], name='tiled_embeddings')
	...
	def discriminator_small(self, image, t_text_embedding, reuse=False):
		...
		h1 = ops.parametric_relu(self.bn1s(ops.conv2d(h0, self.options['df_dim'], d_h=1, d_w=1, name = 'd_h1_conv_s')), name = 'd_pre8') #16
		...
		# ADD TEXT EMBEDDING TO THE NETWORK
		reduced_text_embeddings = ops.parametric_relu(ops.linear(t_text_embedding, self.options['t_dim']/64, 'd_embedding_s'), name = 'd_pre10')
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,1)
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,2)
		tiled_embeddings = tf.tile(reduced_text_embeddings, [1,8,8,1], name='d_tiled_embeddings_s')
	...
	def discriminator_mid(self, image, t_text_embedding, reuse=False):
		...
		h2 = ops.parametric_relu(self.bn2m(ops.conv2d(h1, self.options['df_dim']*2, d_h=1, d_w=1, name = 'd_h2_conv_m')), name = 'd_pre14') #8
		h3 = ops.parametric_relu( self.bn3m(ops.conv2d(h2, self.options['df_dim']*4, name = 'd_h3_conv')), name = 'd_pre15') #4
		# ADD TEXT EMBEDDING TO THE NETWORK
		reduced_text_embeddings = ops.parametric_relu(ops.linear(t_text_embedding, self.options['t_dim']/64, 'd_embedding_m'), name = 'd_pre16')
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,1)
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,2)
		tiled_embeddings = tf.tile(reduced_text_embeddings, [1,16,16,1], name='tiled_embeddings')
		
		h3_concat = concat(3, [h0, tiled_embeddings], name='h3_concat_m')
