...
class GAN:
	...
	def build_model(self):
		...
		g_loss_full = 1-tf.reduce_mean(disc_fake_image_logits)
		
		d_loss1 = 1-tf.reduce_mean(disc_real_image_logits)
		d_loss2 = tf.reduce_mean(disc_wrong_image_logits)
		d_loss3 = tf.reduce_mean(disc_fake_image_logits)

		
		
		g_loss_small = 1-tf.reduce_mean(disc_small_fake_image_logits)
		
		d_loss1_small = 1-tf.reduce_mean(disc_small_real_image_logits)
		d_loss2_small = tf.reduce_mean(disc_small_wrong_image_logits)
		d_loss3_small = tf.reduce_mean(disc_small_fake_image_logits)
		
		
		g_loss_mid = 1-tf.reduce_mean(disc_mid_fake_image_logits)

		d_loss1_mid = 1-tf.reduce_mean(disc_mid_real_image_logits)
		d_loss2_mid = tf.reduce_mean(disc_mid_wrong_image_logits)
		d_loss3_mid = tf.reduce_mean(disc_mid_fake_image_logits)
	...
	def minibatch_discriminate(self, input, num_kernels=5, kernel_dim=3):
		x = ops.linear(input, num_kernels * kernel_dim, self.d_name())
	...
	def add_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 20):
		...
		if z_concat is not None:
			text_augment = tf.reshape(ops.parametric_relu(bn1(ops.noised(
				ops.linear(z_concat,  s/2* s/2* text_filters,self.g_name()))), name = self.g_name()),[-1, s/2, s/2, text_filters])
			
			text_augment = ops.parametric_relu(bn0(ops.deconv2d(text_augment,
				[self.options['batch_size'], s, s, hidden_text_filters], name=self.g_name())), self.g_name())
		...
		residual = ops.deconv2d(ops.parametric_relu(res_hidden, name = self.g_name()),
			[self.options['batch_size'], s, s, filters], k_h=k_h, k_w=k_h, d_h=1, d_w=1, name=self.g_name())
		
		next_layer = prev_layer + residual
		return ops.parametric_relu(next_layer, name=self.g_name())
	...
	def add_ld_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 20):
		...
		if z_concat is not None:
			...
			text_augment = ops.parametric_relu(bn0(ops.deconv2d(text_augment,
				[self.options['batch_size'], s/2, s/2, hidden_text_filters], name=self.g_name())), self.g_name())
	...
	def generator(self, t_z, t_text_embedding):
		...
		h1 = ops.deconv2d(h1, [self.options['batch_size'], s8, s8, self.options['gf_dim']*4],d_h=1, d_w=1, name='g_h1')
		h1 = ops.parametric_relu(self.g_bn1(h1), name = 'g_pre43234')
		#h1 = ops.deconv2d(h0, [self.options['batch_size'], s8, s8, self.options['gf_dim']*4], name='g_h1')
		...
		h2 = ops.deconv2d(h2, [self.options['batch_size'], s4, s4, self.options['gf_dim']*2],d_h=1, d_w=1, name='g_h2')
		h2 = ops.parametric_relu(self.g_bn2(h2), name = 'g_pre432')
		
		h2 = self.add_residual(h2, z_concat)
		h2 = self.add_residual(h2, None)

		h3 = tf.image.resize_nearest_neighbor(h2, [s2, s2])
		h3 = ops.deconv2d(h3, [self.options['batch_size'], s2, s2, self.options['gf_dim']*1],d_h=1, d_w=1, name='g_h3')
		h3 = ops.parametric_relu(self.g_bn3(h3), name = 'g_pre4534')
	...
	def discriminator(self, image, t_text_embedding, reuse=False):
		...
		h1 = ops.parametric_relu(self.bn1f(ops.noised(ops.conv2d(h0, self.options['df_dim']*2, name = 'd_h1_conv'))), name = 'd_pre2') #16
		h2 = ops.parametric_relu(self.bn2f(ops.noised(ops.conv2d(h1, self.options['df_dim']*4, name = 'd_h2_conv'))), name = 'd_pre3') #8
		h3 = ops.parametric_relu(self.bn3f(ops.noised(ops.conv2d(h2, self.options['df_dim']*8, name = 'd_h3_conv'))), name = 'd_pre4') #4
		
		# ADD TEXT EMBEDDING TO THE NETWORK
		reduced_text_embeddings = ops.parametric_relu(ops.linear(t_text_embedding, 4,#self.options['t_dim']/64
	...
	def discriminator_small(self, image, t_text_embedding, reuse=False):
		if reuse:
			...
			tf.get_variable_scope()._reuse = None
		h0 = ops.parametric_relu(ops.conv2d(ops.noised(image, .5), self.options['df_dim'], name = 'd_h0_conv_s'), name = 'd_pre7') #32
		h1 = ops.parametric_relu(self.bn1s(ops.conv2d(h0, self.options['df_dim'], name = 'd_h1_conv_s')), name = 'd_pre8') #16
		#h2 = ops.parametric_relu(self.bn2s(ops.conv2d(h1, self.options['df_dim']*2, name = 'd_h2_conv_s')), name = 'd_pre9') #8
		# ADD TEXT EMBEDDING TO THE NETWORK
		reduced_text_embeddings = ops.parametric_relu(ops.linear(t_text_embedding, self.options['t_dim']/64, 'd_embeddingsmall'), name = 'd_pre10')
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,1)
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,2)
		...
		h3_new = ops.parametric_relu(self.bn3s(ops.conv2d(h3_concat, self.options['df_dim']*8, 1,1,1,1, name = 'd_h3_conv_new')), name = 'd_pre11') #4
	...
	def discriminator_mid(self, image, t_text_embedding, reuse=False):
		if reuse:
			...
			tf.get_variable_scope()._reuse = None
		h0 = ops.parametric_relu(ops.conv2d(ops.noised(image, .5), self.options['df_dim'], name = 'd_h0_conv_m'), name = 'd_pre12') #32
		h1 = ops.parametric_relu( self.bn1m(ops.conv2d(h0, self.options['df_dim']*2, name = 'd_h1_conv_m')), name = 'd_pre13') #16
		h2 = ops.parametric_relu(self.bn2m(ops.conv2d(h1, self.options['df_dim']*2, name = 'd_h2_conv_m')), name = 'd_pre14') #8
		#h3 = ops.parametric_relu( self.bn3m(ops.conv2d(h2, self.options['df_dim']*4, name = 'd_h3_conv')), name = 'd_pre15') #4
		# ADD TEXT EMBEDDING TO THE NETWORK
		reduced_text_embeddings = ops.parametric_relu(ops.linear(t_text_embedding, self.options['t_dim']/64, 'd_embeddingmid'), name = 'd_pre16')
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,1)
		reduced_text_embeddings = tf.expand_dims(reduced_text_embeddings,2)
		tiled_embeddings = tf.tile(reduced_text_embeddings, [1,4,4,1], name='tiled_embeddings')
		
		h3_concat = concat(3, [h2, tiled_embeddings], name='h3_concat_m')
		h3_new = ops.parametric_relu(self.bn4m(ops.conv2d(h3_concat, self.options['df_dim']*8, 1,1,1,1, name = 'd_h3_conv_new_m')), name = 'd_pre17') #4
