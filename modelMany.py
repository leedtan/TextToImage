...
class GAN:
	...
	def build_model(self):
		...
		with tf.variable_scope('scope_1'):
			...
			disc_1_fake_image, disc_1_fake_image_logits   = self.discriminator1(fake_image1, t_real_caption, reuse = True)
		
		with tf.variable_scope('scope_2'):
			disc_2_real_image, disc_2_real_image_logits   = self.discriminator2(t_real_image, t_real_caption)
			disc_2_wrong_image, disc_2_wrong_image_logits   = self.discriminator2(t_wrong_image, t_real_caption, reuse = True)
			disc_2_fake_image, disc_2_fake_image_logits   = self.discriminator2(fake_image2, t_real_caption, reuse = True)
			
		with tf.variable_scope('scope_3'):
			disc_3_real_image, disc_3_real_image_logits   = self.discriminator3(t_real_image, t_real_caption)
			disc_3_wrong_image, disc_3_wrong_image_logits   = self.discriminator3(t_wrong_image, t_real_caption, reuse = True)
			disc_3_fake_image, disc_3_fake_image_logits   = self.discriminator3(fake_image3, t_real_caption, reuse = True)
			
		with tf.variable_scope('scope_4'):
			disc_4_real_image, disc_4_real_image_logits   = self.discriminator4(t_real_image, t_real_caption)
			disc_4_wrong_image, disc_4_wrong_image_logits   = self.discriminator4(t_wrong_image, t_real_caption, reuse = True)
			disc_4_fake_image, disc_4_fake_image_logits   = self.discriminator4(fake_image4, t_real_caption, reuse = True)
			
		with tf.variable_scope('scope_5'):
			disc_5_real_image, disc_5_real_image_logits   = self.discriminator5(t_real_image, t_real_caption)
			disc_5_wrong_image, disc_5_wrong_image_logits   = self.discriminator5(t_wrong_image, t_real_caption, reuse = True)
			disc_5_fake_image, disc_5_fake_image_logits   = self.discriminator5(fake_image5, t_real_caption, reuse = True)
		'''
		...
		d1_loss3 = tf.reduce_mean(cross_entropy(disc_1_fake_image_logits, tf.zeros_like(disc_1_fake_image)))
		...
		d2_loss3 = tf.reduce_mean(cross_entropy(disc_2_fake_image_logits, tf.zeros_like(disc_2_fake_image)))
		...
		d3_loss3 = tf.reduce_mean(cross_entropy(disc_3_fake_image_logits, tf.zeros_like(disc_3_fake_image)))
		...
		d4_loss3 = tf.reduce_mean(cross_entropy(disc_4_fake_image_logits, tf.zeros_like(disc_4_fake_image)))
		...
		d5_loss3 = tf.reduce_mean(cross_entropy(disc_5_fake_image_logits, tf.zeros_like(disc_5_fake_image)))
		
		#####
		
		d_loss1 = d1_loss1 + d1_loss2 + d1_loss3
		d_loss2 = d2_loss1 + d2_loss2 + d2_loss3
		d_loss3 = d3_loss1 + d3_loss2 + d3_loss3
		d_loss4 = d4_loss1 + d4_loss2 + d4_loss3
		d_loss5 = d5_loss1 + d5_loss2 + d5_loss3
		...
		d_loss = d_loss1 + d_loss2 + d_loss3 + d_loss4 + d_loss5
		...
		checks = {
			'd_loss1': d_loss1,
			'd_loss2': d_loss2,
			'disc_1_real_image_logits' : disc_1_real_image_logits,
			'disc_1_wrong_image_logits' : disc_1_wrong_image,
			'disc_1_fake_image_logits' : disc_1_fake_image_logits
		}
	...
	def add_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 20,name_func=None):
		...
		if z_concat is not None:
			text_augment = tf.reshape(ops.lrelu(bn1(
	...
	def add_ld_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 20,name_func=None):
		...
		res_hidden = bn2(ops.noised(ops.deconv2d(
			concatenated, [self.options['batch_size'], s/2, s/2, filters], k_h=k_h, k_w=k_h, d_h=1, d_w=1, name=name_func())))
	...
	def add_stackGan(self, prev_layer, s2, z_concat, name_func=None):
		...
		prev_layer = self.add_residual(prev_layer, z_concat,name_func=name_func)
		prev_layer = ops.deconv2d(prev_layer, [self.options['batch_size'], s2, s2, self.options['gf_dim']*1], d_h=1, d_w=1,name=name_func())
		prev_layer = ops.lrelu(bn1(prev_layer))
		prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func)
		prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func)
		prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func)
		prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func)
		prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func)
		prev_layer = ops.deconv2d(prev_layer, [self.options['batch_size'], s2, s2, self.options['gf_dim']*1], d_h=1, d_w=1,name=name_func())
		prev_layer = ops.lrelu(bn2(prev_layer))
		return prev_layer
	...
	def generator(self, t_z, t_text_embedding):
		...
		bn12 = self.g_bn()
		bn13 = self.g_bn()
		reduced_text_embedding = ops.lrelu( ops.linear(t_text_embedding, self.options['t_dim'], 'g1_g2_g3_g4_g5_embedding'))
		...
		z_concat = concat(1, [t_z, reduced_text_embedding])
		z_concat2 = concat(1, [t_z, reduced_text_embedding])
		z_concat3 = concat(1, [t_z, reduced_text_embedding])
		z_concat4 = concat(1, [t_z, reduced_text_embedding])
		z_concat5 = concat(1, [t_z, reduced_text_embedding])
		z_ = ops.linear(z_concat, self.options['gf_dim']*8*s16*s16, 'g1_h0_lin')
		h0 = tf.reshape(z_, [-1, s16, s16, self.options['gf_dim'] * 8])
		...
		stack1 = ops.deconv2d(h3,[self.options['batch_size'], s, s, 3], name='g1_h6_out')
		...
		stack2 = self.add_stackGan(h3, s2, z_concat2, name_func=self.g2_name)
		stack2out = ops.deconv2d(stack2, [self.options['batch_size'], s, s, 3],name=self.g2_name())
		stack2out = ops.lrelu(bn2(stack2out))
		stack2out = ops.deconv2d(stack2out, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,name=self.g2_name())
		
		stack3 = self.add_stackGan(stack2, s2, z_concat3, name_func=self.g3_name)
		stack3out = ops.deconv2d(stack3, [self.options['batch_size'], s, s, 3],name=self.g3_name())
		stack3out = ops.lrelu(bn3(stack3out))
		stack3out = ops.deconv2d(stack3out, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,name=self.g3_name())
		
		stack4 = self.add_stackGan(stack3, s2, z_concat4, name_func=self.g4_name)
		stack4out = ops.deconv2d(stack4, [self.options['batch_size'], s, s, 3],name=self.g4_name())
		stack4out = ops.lrelu(bn4(stack4out))
		stack4out = ops.deconv2d(stack4out, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,name=self.g4_name())
		
		stack5 = self.add_stackGan(stack4, s2, z_concat5, name_func=self.g5_name)
		stack5out = ops.deconv2d(stack5, [self.options['batch_size'], s, s, 3],name=self.g5_name())
		stack5out = ops.lrelu(bn5(stack5out))
		stack5out = ops.deconv2d(stack5out, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,name=self.g5_name())
