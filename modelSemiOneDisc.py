...
class GAN:
    ...
    def build_model(self, beta1, beta2, lr):
        ...
        with tf.variable_scope('scope_1'):
            p_real_img_logit, p_real_txt_logit,    = self.discriminator(t_real_image, t_real_caption)
            p_wrong_img_logit, p_wrong_txt_logit   = self.discriminator(t_wrong_image, t_real_caption, reuse = True)
            #p_noise_img, p_noise_img_logit, p_noise_txt, p_noise_txt_logit   = self.discriminator(t_wrong_image, t_real_caption, reuse = True)
            
            
            p_fake1_img_logit, p_fake1_txt_logit   = self.discriminator(fake_image1, t_real_caption, reuse = True)
            p_fake2_img_logit, p_fake2_txt_logit   = self.discriminator(fake_image2, t_real_caption, reuse = True)
            p_fake3_img_logit, p_fake3_txt_logit   = self.discriminator(fake_image3, t_real_caption, reuse = True)
            p_fake4_img_logit, p_fake4_txt_logit   = self.discriminator(fake_image4, t_real_caption, reuse = True)
            p_fake5_img_logit, p_fake5_txt_logit   = self.discriminator(fake_image5, t_real_caption, reuse = True)
        ...
        pos_examples = tf.ones_like(p_fake1_img_logit)
        neg_examples = tf.zeros_like(p_fake1_img_logit)
        d_loss_real = tf.reduce_mean(cross_entropy(p_real_img_logit, pos_examples)) + tf.reduce_mean(cross_entropy(p_real_txt_logit, pos_examples))
        d_loss_wrong = tf.reduce_mean(cross_entropy(p_wrong_img_logit, pos_examples)) + tf.reduce_mean(cross_entropy(p_wrong_txt_logit, neg_examples))
        #d_loss_noise = tf.reduce_mean(cross_entropy(p_noise_img_logit, pos_examples))
        
        d_loss_real_img = tf.reduce_mean(cross_entropy(p_real_img_logit, pos_examples))
        d1_loss_noise = tf.reduce_mean(cross_entropy(p_fake1_img_logit, neg_examples))
        d2_loss_noise = tf.reduce_mean(cross_entropy(p_fake2_img_logit, neg_examples))
        d3_loss_noise = tf.reduce_mean(cross_entropy(p_fake3_img_logit, neg_examples))
        d4_loss_noise = tf.reduce_mean(cross_entropy(p_fake4_img_logit, neg_examples))
        d5_loss_noise = tf.reduce_mean(cross_entropy(p_fake5_img_logit, neg_examples))
        
        g1_loss_noise = tf.reduce_mean(cross_entropy(p_fake1_img_logit, pos_examples)) + gDAE1Loss*LambdaDAE
        g2_loss_noise = tf.reduce_mean(cross_entropy(p_fake2_img_logit, pos_examples)) + gDAE2Loss*LambdaDAE
        g3_loss_noise = tf.reduce_mean(cross_entropy(p_fake3_img_logit, pos_examples)) + gDAE3Loss*LambdaDAE
        g4_loss_noise = tf.reduce_mean(cross_entropy(p_fake4_img_logit, pos_examples)) + gDAE4Loss*LambdaDAE
        g5_loss_noise = tf.reduce_mean(cross_entropy(p_fake5_img_logit, pos_examples)) + gDAE5Loss*LambdaDAE
        
        
        
        d1_loss = d1_loss_noise + tf.reduce_mean(cross_entropy(p_fake1_txt_logit, neg_examples))
        d2_loss = d2_loss_noise + tf.reduce_mean(cross_entropy(p_fake2_txt_logit, neg_examples))
        d3_loss = d3_loss_noise + tf.reduce_mean(cross_entropy(p_fake3_txt_logit, neg_examples))
        d4_loss = d4_loss_noise + tf.reduce_mean(cross_entropy(p_fake4_txt_logit, neg_examples))
        d5_loss = d5_loss_noise + tf.reduce_mean(cross_entropy(p_fake5_txt_logit, neg_examples))
        
        g1_loss = g1_loss_noise + tf.reduce_mean(cross_entropy(p_fake1_txt_logit, pos_examples))
        g2_loss = g2_loss_noise + tf.reduce_mean(cross_entropy(p_fake2_txt_logit, pos_examples))
        g3_loss = g3_loss_noise + tf.reduce_mean(cross_entropy(p_fake3_txt_logit, pos_examples))
        g4_loss = g4_loss_noise + tf.reduce_mean(cross_entropy(p_fake4_txt_logit, pos_examples))
        g5_loss = g5_loss_noise + tf.reduce_mean(cross_entropy(p_fake5_txt_logit, pos_examples))
        ...
        checks = {
            'p_real_img_logit' : p_real_img_logit,
            'p_wrong_img_logit' : p_wrong_img_logit,
            'p_fake1_img_logit' : p_fake1_img_logit
        }
    ...
    def add_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 20,name_func=None):
        ...
        if z_concat is not None:
            text_augment = reshape(ops.lrelu(bn1(
                ops.linear(z_concat,  s/2* s/2* text_filters,name_func()))),[-1, s//2, s//2, int(text_filters)])
