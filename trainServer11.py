...
def main():
    ...
    parser.add_argument('--caption_vector_length', type=int, default=1000,#4096 - zdim (30)#2400Lee
    ...
    parser.add_argument('--save_epoch', type=list, default=[2,5,10],
    ...
    for i in range(args.epochs):
        noise_gen = 1/( np.power(100+i,1/3))
        noise_disc = 1/( np.power(100+i,1/3))
        t = time.time()
        img_idx = 0
        batch_no = 0
        caption_vectors = 0
        while batch_no*args.batch_size < loaded_data['data_length']:
            ...
            if np.random.rand() > 0.7 or init == 0:
                init = 1
                last_img = 'real'
                '''d_gvs, g_gvs, _, _, d_loss, g_loss, g1_loss, g2_loss, g3_loss, g4_loss, g5_loss, \
                    d1_loss, d2_loss, d3_loss, d4_loss, d5_loss, img1, img2,img3,img4,img5   = sess.run(
                    [gan.d_gvs, gan.g_gvs, g_optim, d_optim, loss['d_loss'], loss['g_loss'], 
                ...
                d_gvs = [np.max(np.array([np.abs(x) for x in g])) for g in g_gvs]
                '''
            ...
            if (batch_no % args.save_every) == 0:
                ...
                save_path = saver.save(sess, "Data/Models/latest_model_Server11_{}_temp.ckpt".format(args.data_set))
            if i in args.save_epoch:#i%5 == 0:
                save_path = saver.save(sess, "Data/ModelEval/Server11_after_{}_epoch_{}.ckpt".format(i, args.data_set))
