...
class GAN:
    ...
    def __init__(self, options):
        with tf.variable_scope('seondtestmodelinit'):
    ...
    def build_model(self, beta1, beta2, lr):
        with tf.variable_scope('seondtestmodel'):
            ...
            gvs = optimizer.compute_gradients(
                ...
                var_list=variables['g1_vars'] + variables['g2_vars'] + 
                variables['g3_vars'] + variables['g4_vars'] + variables['g5_vars'])
            capped_gvs = [(tf.clip_by_value(grad, -.01, .01), var) for grad, var in gvs if grad is not None]
    ...
    def add_residual_pre(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 20,name_func=None):
        if name_func is None:
            name_func = self.g_name
        
        s = prev_layer.get_shape()[1].value
        hidden_filters = prev_layer.get_shape()[3].value * 20
        ...
        res_hidden = ops.deconv2d(ops.lrelu(bn3(concatenated), name=name_func()), 
