...
class GAN:
    ...
    def build_model(self, beta1, beta2, lr):
        ...
        outputs = {
            ...
            'img4' : fake_image4,
            'img5' : fake_image5,
            'output_p_5_fake_txt_logit' : p_5_fake_txt_logit,
            'output_p_5_fake_img_logit' : p_5_fake_img_logit
        }
        ...
        optimizer = tf.train.AdamOptimizer(lr, beta1 = beta1,beta2 = beta2)
        gvs = optimizer.compute_gradients(
            d_loss_noise * noise_indicator + d_loss * (1-noise_indicator),
    ...
    def add_residual_pre(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 2,name_func=None):
        if name_func is None:
            name_func = self.g_name
        
        s = prev_layer.get_shape()[1].value
        hidden_filters = 1#prev_layer.get_shape()[3].value * 10
        ...
        if z_concat is not None:
            ...
            text_augment = ops.lrelu(bn0(ops.deconvMany(text_augment,
        ...
        res_hidden = ops.deconvMany(ops.lrelu(bn3(concatenated), name=name_func()), 
            [self.options['batch_size'], s, s, hidden_filters], k_h=k_h, k_w=k_h, d_h=1, d_w=1, name=name_func())
        
        residual = ops.deconv2d(ops.lrelu(bn4(concatenated), name=name_func()), 
    ...
    #Residual Block
    def add_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 2,name_func=None):
        if name_func is None:
            name_func = self.g_name
        ...
        res_hidden = bn2(ops.deconvMany(concatenated, 
            [self.options['batch_size'], s, s, filters], k_h=k_h, k_w=k_h, d_h=1, d_w=1, name=name_func()))
    ...
    def add_ld_residual(self, prev_layer, z_concat, text_filters = 3, k_h = 5, k_w = 5, hidden_text_filters = 2,name_func=None):
        if name_func is None:
            name_func = self.g_name
    ...
    def add_stackGan(self, prev_layer, s2, z_concat, name_func=None):
        ...
        #prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func)
        prev_layer = self.add_residual_standard(prev_layer, z_concat=None,name_func=name_func,k_h = 3, k_w = 3)
        return prev_layer
    ...
    def add_stackGanImg(self, prev_layer, s2, z_concat, name_func=None):
        ...
        prev_layer = self.add_residual_pre(prev_layer, z_concat=None,name_func=name_func)
        prev_layer = self.add_residual_pre(prev_layer, z_concat=None,name_func=name_func)
        prev_layer = self.add_residual_pre(prev_layer, z_concat=None,name_func=name_func,k_h = 3, k_w = 3)
        return prev_layer
    ...
    def generator(self, t_z, t_text_embedding):
        ...
        bn12 = self.g_bn()
        bn13 = self.g_bn()
        z_rec_target = ops.noised(concat(1, [t_z, t_text_embedding]), self.noise_gen)
        reduced_text_embedding = ops.lrelu( ops.linear(t_text_embedding, self.options['t_dim'], 'g1_embedding'))
        ...
        h1 = ops.deconvMany(h0, [self.options['batch_size'], s8, s8, self.options['gf_dim']*4],name='g1_h1')
        h1 = ops.lrelu(self.g_bn1(h1), name = 'g1_pre43234')

        h1 = self.add_residual_standard(h1, z_concat)

        h2 = ops.deconvMany(h1, [self.options['batch_size'], s4, s4, self.options['gf_dim']*2],name='g1_h2')
        h2 = ops.lrelu(self.g_bn2(h2), name = 'g1_pre432')

        h2 = self.add_residual_standard(h2, z_concat)

        h3 = ops.deconvMany(h2, [self.options['batch_size'], s2, s2, self.options['gf_dim']*1],name='g1_h3')
        h3 = ops.lrelu(self.g_bn3(h3), name = 'g1_pre4534')

        h3 = self.add_residual_standard(h3, z_concat)

        stack1 = ops.deconvMany(h3,[self.options['batch_size'], s, s, self.options['gf_dim']/4], name='g1_h6_out')
        stack1 = ops.lrelu(self.g_bn4(stack1))
        
        stack1out = ops.deconv2d(stack1, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,k_h=3, k_w=3,name=self.g_name())
        
        stack2 = self.add_stackGan(ops.noised_gamma(stack1, self.noise_gen), s, z_concat2, name_func=self.g2_name)

        stack2out = ops.deconv2d(stack2, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,k_h=3, k_w=3,name=self.g2_name())
        
        stack3 = self.add_stackGan(ops.noised_gamma(stack2, self.noise_gen), s, z_concat3, name_func=self.g3_name)

        stack3out = ops.deconv2d(stack3, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,k_h=3, k_w=3,name=self.g3_name())
        
        stack4 = self.add_stackGan(ops.noised_gamma(stack3, self.noise_gen), s, z_concat4, name_func=self.g4_name)

        stack4out = ops.deconv2d(stack4, [self.options['batch_size'], s, s, 3], d_h=1, d_w=1,k_h=3, k_w=3,name=self.g4_name())
        
        stack5out = self.add_stackGanImg(ops.noised_gamma(stack4out, self.noise_gen), s, z_concat5, name_func=self.g5_name)
        ...
        return (tf.tanh(stack1out)/2. + 0.5), (tf.tanh(stack2out)/2. + 0.5), (tf.tanh(stack3out)/2. + 0.5), (tf.tanh(stack4out)/2. + 0.5), (tf.tanh(stack5out)/2. + 0.5), \
                ...
                tf.reduce_mean(tf.square(z_rec4 - z_rec_target)),0
