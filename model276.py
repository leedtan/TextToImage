...
class GAN:
	...
	def build_model(self):
		...
		d_loss_fake_full = d_loss3
		d_loss_fake_mid = d_loss3_mid
		d_loss_fake_small = d_loss3_small
		...
		loss = {
			...
			'd_loss_full' : d_loss_fake_full,
			'g_loss_mid' : g_loss_mid,
			'd_loss_mid' : d_loss_fake_mid,
			'g_loss_small' : g_loss_small,
			'd_loss_small' : d_loss_fake_small,
		...
		checks = {
			...
			'd_loss2': d_loss2,
			'd_loss3' : d_loss3,
			'd_loss_fake_full' : d_loss_fake_full,
			'd_loss_fake_mid' : d_loss_fake_mid,
			'd_loss_fake_small' : d_loss_fake_small,
			'g_loss_full' : g_loss_full,
			'g_loss_mid' : g_loss_mid,
			'g_loss_small' : g_loss_small,
	...
	def generator(self, t_z, t_text_embedding):
		...
		h2res = ops.deconv2d(h2, [self.options['batch_size'], s4, s4, 1],d_h=1, d_w=1, name='g_h2res')
		h2res = ops.parametric_relu(self.g_bn2h(h2res))
		h2resflat = tf.reshape(h2res, [-1, s4*s4])
	...
	def discriminator_small(self, image, t_text_embedding, reuse=False):
		...
		h2 = ops.parametric_relu( self.d_bn2(ops.conv2d(h1, self.options['df_dim'], d_h=1, d_w=1, name = 'd_h2_conv'))) #8
