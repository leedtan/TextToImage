...
def main():
	...
	parser.add_argument('--learning_rate', type=float,default=.001,#1e-4 or 1e-5LEECHANGE default=0.0002,
					   help='Learning Rate')
	...
	gan = modelstable.GAN(model_options)
	...
	'''
	d_optim = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['d_loss'], var_list=variables['d_vars'])
	d_optim_full = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['d_loss_full'], var_list=variables['d_vars'])
	d_optim_small = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['d_loss_small'], var_list=variables['d_vars'])
	d_optim_mid= tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['d_loss_mid'], var_list=variables['d_vars'])
	'''
	...
	'''
	g_optim = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['g_loss'], var_list=variables['g_vars'])
	g_optim_full = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['g_loss_full'], var_list=variables['g_vars'])
	g_optim_mid = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['g_loss_mid'], var_list=variables['g_vars'])
	g_optim_small = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(loss['g_loss_small'], var_list=variables['g_vars'])
	'''
	optimizer = tf.train.AdamOptimizer(learning_rate=args.learning_rate, beta1 = args.beta1)
	...
	for i in range(args.epochs):
		...
		while batch_no*args.batch_size < loaded_data['data_length']:
			...
			if i > 1000:
				sess.run(gan.wgan_clip_1)
			elif i > 100:
				sess.run(gan.wgan_clip_1eneg1)
			else:
				sess.run(gan.wgan_clip_1eneg2)
			'''
			...
			if batch_no % 2 == 0:
				...
				img_mid = gen_mid[idx,:,:,:]
				r_mid = real_mid[idx,:,:,:]
				scipy.misc.imsave('images_stable4/' + str(i) + '_img_idx:' + str(batch_no) + 'full.jpg',img_full)
				scipy.misc.imsave('images_stable4/' + str(i) + '_img_idx:' + str(batch_no) + 'tiny.jpg',img_small)
				scipy.misc.imsave('images_stable4/' + str(i) + '_img_idx:' + str(batch_no) + 'mid.jpg',img_mid)
				scipy.misc.imsave('images_stable4/' + str(i) + '_img_idx:' + str(batch_no) + 'a_r_small.jpg',r_small)
				scipy.misc.imsave('images_stable4/' + str(i) + '_img_idx:' + str(batch_no) + 'a_r_mid.jpg',r_mid)
			...
			if (batch_no % args.save_every) == 0:
				...
				save_path = saver.save(sess, "Data/Models/latest_stable4_{}_temp.ckpt".format(args.data_set))
		if i%5 == 0:
