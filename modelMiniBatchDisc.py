...
class GAN:
	...
	def discriminator(self, image, t_text_embedding, reuse=False):
		...
		mini_information = self.minibatch_discriminate_full(tf.reshape(
			image, [self.options['batch_size'], -1]), num_kernels=20, reuse = reuse)
		...
		h3_new = ops.parametric_relu(ops.conv2d(h3_concat, self.options['df_dim']*8, 1,1,1,1, name = 'd_h3_conv_new'), name = 'd_pre6') #4
	...
	def discriminator_small(self, image, t_text_embedding, reuse=False):
		...
		h3_new = ops.parametric_relu(ops.conv2d(h3_concat, self.options['df_dim']*8, 1,1,1,1, name = 'd_h3_conv_new'), name = 'd_pre11') #4
	...
	def discriminator_mid(self, image, t_text_embedding, reuse=False):
		...
		h3_new = ops.parametric_relu(ops.conv2d(h3_concat, self.options['df_dim']*8, 1,1,1,1, name = 'd_h3_conv_new_m'), name = 'd_pre17') #4
